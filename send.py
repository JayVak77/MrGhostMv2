...
def send_email(to_email, body, count):
    ...
    if config.get("use_single_smtp", False):
        # Parse the single SMTP string
        smtp_server_config = config.get("smtp_server").split('|')
        host = smtp_server_config[0]
        username = smtp_server_config[1]
        password = smtp_server_config[2]
        port = int(smtp_server_config[3])
        use_tls = smtp_server_config[4].lower() == 'true'
        ...
        #     'use_tls': use_tls
        # }
        # send_via_smtp(smtp_config, message)
    # else:
        # Multiple SMTP servers
        # smtp_list = config.get("smtp_servers", [])
        # for smtp_str in smtp_list:
        #     host, username, password, port, use_tls_str = smtp_str.split('|')
        #     port = int(port)
        #     use_tls = use_tls_str.lower() == 'true'
        #     smtp_config = {
        ...
        #         'use_tls': use_tls
        #     }
        #     # you can decide to send via each SMTP, or pick one
        #     send_via_smtp(smtp_config, message)
        # If using multiple SMTP servers
    if 'smtp_servers' in config:
        smtp_servers = config['smtp_servers']
        for smtp_server in smtp_servers:
            host, port, user, password, use_tls_str = smtp_server.split('|')
            port = int(port)
            use_tls = use_tls_str.lower() == 'true'
            smtp_config = {
                'host': host,
...
def send_via_smtp(smtp_config, message):
    try:
